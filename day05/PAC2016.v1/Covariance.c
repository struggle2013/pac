...
#ifdef _PADDING
inline void COV_manual(double *mt1, double *mt2, double *val) {
#else
...
#ifdef _PADDING
//	register __m256d value = _mm256_load_pd(val);
#endif
#pragma unroll(8)
	for (i=0; i<RB ; i+=8) { 
...
#ifdef _PADDING
//	value = _mm256_add_pd(value, _mm256_set_pd(_mm512_reduce_add_pd(c),_mm512_reduce_add_pd(x2),
//				_mm512_reduce_add_pd(x1), _mm512_reduce_add_pd(nm)));
//	_mm256_store_pd((void*)val, value);
    val[0] += _mm512_reduce_add_pd(nm);
	val[1] += _mm512_reduce_add_pd(x1);
	val[2] += _mm512_reduce_add_pd(x2);
	val[3] += _mm512_reduce_add_pd(c);
#else
...
#endif
#ifdef _PADDING
	REALTYPE *VAL = (REALTYPE *)_mm_malloc(4*nvar_l*nv_i*sizeof(REALTYPE),64);
    bzero((void*)VAL, 4*nvar_l*nv_i*sizeof(REALTYPE));
#else
#ifdef _USEHBW
...
#pragma vector aligned
    ...
    for (IVar1=svar; IVar1<evar; IVar1++) {
    	for (IVar2=0; IVar2<nv_i; IVar2++) {
#ifdef _PADDING
            REALTYPE IRep = VAL[j]<=0.0 ? 0.0 : 1.0/VAL[j];
            COV[IVar2*nvar_i+IVar1]=(VAL[j+3]-VAL[j+1]*VAL[j+2]*IRep)*IRep;
			j+=4;
#else
