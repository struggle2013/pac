...
!$OMP PARALLEL DO private(IVar1,IVar2,I2) shared(NVAR,COV,MType)
    ...
    !COV(IVar1, IVar2) = Covariance(NROW, MType(IVar1,:), &
        DO IVar2 = 1, NV
            ...
            Covariance=0.0
            DO IObs=1, NROW
                IF(IsMissingPheno(MType(IVar1,IObs)) .OR. &
                    IsMissingPheno(MType(I2,IObs)) THEN
                NumMissing=NumMissing+1

                ELSE
                MeanX1=MeanX1+MType(IVar1,IObs)
                MeanX2=MeanX2+MType(I2,IObs)

                Covariance=Covariance+MType(IVar1,IObs)*MType(I2,IObs)
